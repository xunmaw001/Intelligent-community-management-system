<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="值班区域" prop="zhibanquyu">
          <el-input v-model="ruleForm.zhibanquyu" 
              placeholder="值班区域" clearable  :readonly="ro.zhibanquyu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="值班区域" prop="zhibanquyu">
              <el-input v-model="ruleForm.zhibanquyu" 
                placeholder="值班区域" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="班次" prop="banci">
          <el-select v-model="ruleForm.banci" placeholder="请选择班次">
            <el-option
                v-for="(item,index) in banciOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="值班内容" prop="zhibanneirong">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="值班内容"
                  v-model="ruleForm.zhibanneirong" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.zhibanneirong" label="值班内容" prop="zhibanneirong">
                    <span>{{ruleForm.zhibanneirong}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.yuangongxingming = true;
          ...
          if(o=='zhibanquyu'){
            this.ruleForm.zhibanquyu = obj[o];
	    this.ro.zhibanquyu = true;
            continue;
          }
          if(o=='banci'){
	    ...
	    this.ro.banci = true;
          ...
          if(o=='zhibanneirong'){
            this.ruleForm.zhibanneirong = obj[o];
	    this.ro.zhibanneirong = true;
            continue;
          }
          if(o=='kaishishijian'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/wuyeyuangong/gonghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
    ...
    gonghaoChange () {
      this.$http({
        url: `follow/wuyeyuangong/gonghao?columnValue=`+ this.ruleForm.gonghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zhibanxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zhibanxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
