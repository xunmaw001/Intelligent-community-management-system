<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="投诉项目" prop="tousuxiangmu">
          <el-input v-model="ruleForm.tousuxiangmu" 
              placeholder="投诉项目" clearable  :readonly="ro.tousuxiangmu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="投诉项目" prop="tousuxiangmu">
              <el-input v-model="ruleForm.tousuxiangmu" 
                placeholder="投诉项目" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="投诉原因" prop="tousuyuanyin">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="投诉原因"
                  v-model="ruleForm.tousuyuanyin" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.tousuyuanyin" label="投诉原因" prop="tousuyuanyin">
                    <span>{{ruleForm.tousuyuanyin}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.loufanghao = true;
          ...
          if(o=='tousuxiangmu'){
            this.ruleForm.tousuxiangmu = obj[o];
	    this.ro.tousuxiangmu = true;
            continue;
          }
          if(o=='tousuneirong'){
	    ...
	    this.ro.tousuneirong = true;
          ...
          if(o=='tousuyuanyin'){
            this.ruleForm.tousuyuanyin = obj[o];
	    this.ro.tousuyuanyin = true;
            continue;
          }
          if(o=='tousuriqi'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.yezhuzhanghao!=''&&json.yezhuzhanghao){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yezhutousu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yezhutousuCrossAddOrUpdateFlag = false;
                  this.parent.search();
