<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="朝向" prop="chaoxiang">
          <el-select v-model="ruleForm.chaoxiang" placeholder="请选择朝向">
            <el-option
                v-for="(item,index) in chaoxiangOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="合同文件" prop="hetongwenjian">
          <file-upload
          tip="点击上传合同文件"
          action="file/upload"
          :limit="1"
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="装修情况" prop="zhuangxiuqingkuang">
                <el-input
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.zhuangxiuqingkuang" label="装修情况" prop="zhuangxiuqingkuang">
                    <span>{{ruleForm.zhuangxiuqingkuang}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.chaoxiangOptions = "东,西,南,北".split(',')
    },
    // 下二随
    yezhuzhanghaoChange () {
      this.$http({
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `fangchanxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.fangchanxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
